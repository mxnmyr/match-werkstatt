    ...
    if (!fs.existsSync(orderFolderPath)) {
      ...
      try {
        ...
        const order = await ordersCollection.findOne(
          ...
          { projection: { networkFolderCreated: 1 } }
        );
...
app.get('/api/orders', async (req, res) => {
  try {
    ...
    // Lade Note History für jede Order
    ...
    // Erweitere Orders mit Relations
    const enrichedOrders = await Promise.all(orders.map(async (order) => {
      // Note History laden
      const noteHistory = await noteHistoryCollection.find({ 
        orderId: new ObjectId(order._id) 
      })
      .sort({ createdAt: -1 })
      .toArray();
      
      // Documents laden (falls nicht im Order-Objekt)
      let documents = order.documents || [];
      if (documents.length === 0) {
        documents = await documentsCollection.find({ 
          orderId: new ObjectId(order._id) 
        }).toArray();
      }
      
      // Components laden
      const components = await componentsCollection.find({ 
        orderId: new ObjectId(order._id) 
      }).toArray();
...
app.get('/api/orders/:id', async (req, res) => {
  try {
    ...
    // Component Documents laden
    const enrichedComponents = await Promise.all(components.map(async (component) => {
      const compDocuments = await documentsCollection.find({ 
  ...
  try {
    ...
    try {
      ...
      if (documents !== undefined) {
        // Dokumente mit type-Feld ergänzen
      ...
      // Hole den aktualisierten Auftrag direkt von MongoDB zurück
      const updatedOrder = await collection.findOne(
        { _id: new ObjectId(req.params.id) }
      );

      console.log('MongoDB query result:', updatedOrder ? 'Found' : 'Not found');
      
      if (!updatedOrder) {
        return res.status(404).json({ error: 'Order not found after update' });
      }

      // Konvertiere MongoDB ObjectId zu String für Frontend-Kompatibilität
      const responseOrder = {
        ...updatedOrder,
      ...
      console.log('Final response documents:', responseOrder.documents);
...
app.get('/api/admin/network-config', async (req, res) => {
  try {
    ...
    let networkPathConfig = null;
    if (db) {
      networkPathConfig = await db.collection('SystemConfig').findOne({ key: 'NETWORK_BASE_PATH' });
    }
    ...
    res.json({
      success: true,
      configured: currentNetworkPath !== 'PLATZHALTER_NETZWERKPFAD',
      networkPath: currentNetworkPath,
      basePath: currentNetworkPath,
      accessible: currentNetworkPath !== 'PLATZHALTER_NETZWERKPFAD' ? fs.existsSync(currentNetworkPath) : false,
      message: currentNetworkPath === 'PLATZHALTER_NETZWERKPFAD' 
        ? 'Netzwerkpfad nicht konfiguriert. Setzen Sie die Umgebungsvariable NETWORK_BASE_PATH.' 
        : 'Netzwerkpfad konfiguriert',
      description: networkPathConfig?.description || ''
    });
  } catch (error) {
    console.error('Fehler beim Laden der Netzwerkpfad-Konfiguration:', error);
    res.status(500).json({ 
      success: false,
      error: 'Fehler beim Laden der Netzwerkpfad-Konfiguration',
      details: error.message 
    });
  }
});

// Netzwerkpfad-Konfiguration setzen/aktualisieren (für Admin)
app.post('/api/admin/network-config', async (req, res) => {
  try {
    ...
    if (!networkPath || typeof networkPath !== 'string') {
      return res.status(400).json({ 
        success: false,
        error: 'Netzwerkpfad ist erforderlich und muss ein String sein' 
      });
    }
    ...
    if (!fs.existsSync(networkPath)) {
      return res.status(400).json({ 
        success: false,
        error: `Netzwerkpfad nicht erreichbar: ${networkPath}`,
        accessible: false
      });
    }
    ...
    // Testschreibzugriff
    try {
      const testFile = path.join(networkPath, 'test_write_access.tmp');
      fs.writeFileSync(testFile, 'test');
      fs.unlinkSync(testFile);
    } catch (writeError) {
      return res.status(400).json({ 
        success: false,
        error: `Kein Schreibzugriff auf Netzwerkpfad: ${networkPath}`,
        writeAccess: false
      });
    }

    // MongoDB-Verbindung für Konfiguration
    const mongoClient = new MongoClient('mongodb://localhost:27017');
    await mongoClient.connect();
    const db = mongoClient.db('matchdb');
    
    // Konfiguration in MongoDB speichern/aktualisieren
    await db.collection('SystemConfig').updateOne(
      ...
      { 
        $set: { 
          key: 'NETWORK_BASE_PATH',
          value: networkPath,
          description: 'Basis-Netzwerkpfad für Auftragsordner',
          updatedAt: new Date(),
          userId: 'system'
        }
      },
    ...
    res.json({
      success: true,
      message: 'Netzwerkpfad erfolgreich konfiguriert',
    ...
    console.error('Fehler beim Setzen der Netzwerkpfad-Konfiguration:', error);
    res.status(500).json({ 
      success: false,
      error: 'Fehler beim Setzen der Netzwerkpfad-Konfiguration',
      details: error.message 
    });
  }
});
